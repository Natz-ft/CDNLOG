...
public class ZTE_CDN_LOG_UNZIP_THREAD extends Thread { 
	...
	public ZTE_CDN_LOG_UNZIP_THREAD(String name) {
		super(name);
	}
  ...
  //String sql = "select fileName,localpath from cdn_zte_log_sftp where ( status='' or status is null ) order by fileName    ";
	public static int UNZIP_Thread() throws Exception {
		...
		String sql = "select fileName,localpath from cdn_zte_log_sftp where ( status='' or status is null ) order by fileName limit " 	+ String.valueOf(Thread_num);
		...
		if (db.createConnLocal()) {
			...
			while (db.next()) {
				...
				jt = new ZTE_CDN_LOG_UNZIP_THREAD(path + "," + fileName);
